...
const ML_PK_FRAC = { MTD: 1 / 12, QTD: 0.25, YTD: 0.72, 'Rolling 12': 1 };
  ...
  ML_REGIONS.forEach(region => {
    ...
    t.ext.forEach((ext, ei) => {
      ...
      for (let i = 0; i < 48; i++) {
        ...
        const oppIn = Math.round(ML_REGION_SCALE[region] * (6 + size * 300) * 10) / 10;
        ...
        const sales = {
          MTD: Math.round(r12K / 12 * (0.3 + r() * 1.7)),
          QTD: Math.round(r12K / 4 * (0.45 + r() * 1.2)),
          YTD: Math.round(r12K * (0.6 + r() * 0.3)),
          'Rolling 12': r12K,
        };
        const prodCat = r12K === 0 ? 'Prospect' : r12K < 400 ? 'Dabbler' : 'Producer';
        ...
        out.push({
          id: `ml-${seed}`, region, ext, int, spec, type, firm, channel,
          ...
          addr: head * (0.45 + r() * 0.55),
        });
      }
    });
  });
  ML_REGIONS.forEach(rg => {
    const list = out.filter(x => x.region === rg).sort((a, b) => b.addr - a.addr);
...
const mlCatOf = (k) => (k <= 0 ? 'Prospect' : k < 400 ? 'Dabbler' : 'Producer');
...
const ML_DIMS = {
  ...
  segment:  { label: 'Segment',           of: r => r.segment, order: () => LV_SEGMENTS, l3: true, label_: k => `Segment ${k}`, color: k => LV_SEG_META[k].dot },
};
...
function mlMixW(row, xf, skip) {
  const f = ['vehicle', 'cat', 'product'].filter(k => k !== skip && xf[k].length);
...
function mlEmpty() {
  return { n: 0, opp: 0, yours: 0, sales: 0, inflow: 0, prior: 0, acts: 0, covered: 0, stale: 0, producers: 0, dabblers: 0, prospects: 0, eng: 0, engaged: 0,
    sigN: 0, sigOpp: 0, sigAct: 0, sigMet: 0, sigWon: 0, wonK: 0, confW: 0, segA: 0, segAcov: 0, fam: {} };
}
function mlAdd(a, row, w, ctx) {
  const M = LV_MEASURES[ctx.measure];
  a.n += 1;
  a.opp += row[M.opp] * w; a.yours += row[M.yours] * w;
  a.sales += mlActual(row, ctx.measure, ctx.pk) * w;
  ...
  a[row.prodCat === 'Producer' ? 'producers' : row.prodCat === 'Dabbler' ? 'dabblers' : 'prospects'] += 1;
  a.eng += row.eng * w; if (row.eng > 0) a.engaged += 1;
...
function mlBuckets(rows, dim, ctx, xf) {
  ...
  rows.forEach(r => {
    if (d.mix) {
      const other = ['vehicle', 'cat', 'product'].filter(k => k !== d.mix && xf && xf[k].length);
      const acc = {};
...
const ML_METRICS = {
  ...
  perFA:     { label: 'Activities / FA',   get: a => mlPct(a.acts, a.n),                 fmt: v => v.toFixed(1) },
  coverage:  { label: 'Coverage %',        get: a => mlPct(a.covered, a.n),              fmt: v => `${Math.round(v * 100)}%` },
  engaged:   { label: 'Engaged %',         get: a => mlPct(a.engaged, a.n),              fmt: v => `${Math.round(v * 100)}%` },
